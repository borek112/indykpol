...
import { DEMO_STANDARD_CODES } from "./seed/constants";
...
export const nutritionRouter = createRouter({
  listStandards: publicQuery.query(async () => {
    return getDb().select().from(s.nutritionalStandards)
      .where(and(isNull(s.nutritionalStandards.companyId), inArray(s.nutritionalStandards.code, [...DEMO_STANDARD_CODES])))
      .orderBy(s.nutritionalStandards.phase, s.nutritionalStandards.gender);
  }),
