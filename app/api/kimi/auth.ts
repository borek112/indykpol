...
async function exchangeAuthCode(
  ...
  const kimiAuthUrl = getKimiAuthUrl();
  ...
  const resp = await fetch(`${kimiAuthUrl}/api/oauth/token`, {
    method: "POST",
...
function getJwks() {
  ...
  return jose.createRemoteJWKSet(
    new URL(`${kimiAuthUrl}/api/.well-known/jwks.json`),
  );
}
...
export async function authenticateRequest(headers: Headers): Promise<User> {
  ...
  if (!env.isProduction) {
    try {
      ...
      console.warn("[auth] User lookup failed in dev mode; using fallback user:", error);
    }

    const fallbackUser: User = {
      id: 0,
      unionId: claim.unionId,
      name: claim.unionId === (env.ownerUnionId || "dev-owner") ? "Developer" : claim.unionId,
      email: null,
      avatar: null,
      role: "admin",
      companyId: null,
