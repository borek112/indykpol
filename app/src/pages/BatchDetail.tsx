...
function BatchAnalyticsCenter({ batch, batchId, onClose }: { batch: any; batchId: number; onClose: () => void }) {
  ...
  const cards = [
    ...
    ["Wiek", `${ageDays} dni`], ["Upadki", value(daily.data?.totals?.mortality, `${fmtNum(daily.data?.totals?.mortality ?? 0)} szt.`)],
    ["Śr. masa", value(batch.avgWeightG, `${(batch.avgWeightG / 1000).toFixed(2)} kg`)], ["FCR", value(batch.fcr, batch.fcr?.toFixed(2))],
    ["Pasza", value(daily.data?.totals?.feedKg, `${fmtNum(daily.data?.totals?.feedKg ?? 0)} kg`)], ["Woda", value(daily.data?.totals?.waterLiters, `${fmtNum(daily.data?.totals?.waterLiters ?? 0)} l`)],
